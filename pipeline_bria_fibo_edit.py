...
class BriaFiboEditPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def __call__(
        ...
        self.transformer = BriaFiboTransformer2DModel.from_pretrained(os.path.join(self.custom_model_path, "transformer"), device_map="cpu", torch_dtype=torch_dtype)
        ...
        print("[Fibo Edit] Unloading text encoder to save VRAM")
        self.text_encoder.to("cpu")  # Unload text encoder from memory
        ...
        if output_type == "latent":
            ...
            print("[Fibo Edit] Unloading transformer and loading VAE")
            del self.transformer
            torch.cuda.empty_cache()
            self.vae.to(latents_device)
